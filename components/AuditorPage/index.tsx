...
export const AuditorPage = () => {
  // const { data: contestData } = useContestInfo();
  //
  // const { address } = useAccount({});
  // const [pendingNFTS, setPendingNFTS] = useState<
  //   { nftId: number; competitionName: string; pendingAmount: number }[] | null
  // >(null);
  ...
  // });
  //
  // const [telegram, twitter, github] =
  //   (readData?.[2].result as `0x${string}`[])?.map((x) =>
  //     parseBytes32String(x),
  //   ) ?? [];
  //
  // useEffect(() => {
  ...
  //   }
  //
  //   if (!telegramUsername) setTelegramUsername(telegram);
  //
  //   if (!twitterUsername) setTwitterUsername(twitter);
  //
  //   if (!githubUsername) setGithubUsername(github);
  // }, [
  //   readData,
  //   github,
  //   githubUsername,
  ...
  //
  // const auditorResult = contestData?.auditorResults?.find(
  //   (x) => x.address == props.address,
  // );
  // const globalTop = contestData?.auditorResults
  //   ?.slice()
  //   .sort((a, b) => (a.total < b.total ? 1 : a.total > b.total ? -1 : 0))
  //   .findIndex((x) => x.address == props.address);
  //
  // let sum = 0;
  // const data = {
  //   labels: auditorResult?.competitions.map(
  //     (x) => `${x.id} ${guessCompetitionName(x.id)}`,
  //   ),
  //   datasets: [
  //     {
  //       label: "Auditor's global score",
  //       data: auditorResult?.competitions.map(
  //         (x) => (sum = (sum || 0) + x.amount * x.weight),
  //       ),
  ...
  //     ethers.utils.parseBytes32String(param),
  //   );
  //
  //   if (scores.length == 0) {
  //     return `No results`;
  ...
  //       <p>
  //         Auditor statistics:{' '}
  //         {props.address == 'me' ? <b>Wallet not connected</b> : props.address}
  //       </p>
  //       <p>
  //         Global top: {globalTop + 1} {getMedal(globalTop)}. Competition medals:{' '}
  //         {auditorResult?.competitions.map((x) => {
  //           const topPlace = competitionTops
  //             .find((c) => c.id == x.id)
  //             ?.top.findIndex((t) => t.address == props.address);
  //           return <>{getMedal(topPlace!)}</>;
  //         })}
  //       </p>
  //       <ChartWrapper>
  ...
  //             [{x.id} {guessCompetitionName(x.id)}] {x.amount}
  //             <div style={{ display: 'inline', color: '#777777' }}>
  //               x{x.weight}
  //             </div>{' '}
  //             score. Top – {topPlace! + 1} {getMedal(topPlace!)}.<br />
  //             <div style={{ display: 'inline', color: '#777777' }}>
  //               {formatMetadata(x.params)}
  //             </div>
  //             <br />
  //             <br />
  //           </div>
  ...
  //               const airdropRequest = await fetch(
  //                 `/api/faucet?auditor=${address}&nftId=${auditorResult?.competitions[0].id}`,
  //               );
  ...
  //               else setAirdropTxHash(airdropJson.hash);
  //             }}
  //           >
  //             {!airdropTxHash && (
  ...
  //                   onChange={(x) => setTelegramUsername(x.target.value)}
  //                 />
  //               </Socials>
  //               <Socials>
  //                 <Image
  ...
  //                   onChange={(x) => setTwitterUsername(x.target.value)}
  //                 />
  //               </Socials>
  //               <Socials>
  //                 <Image
  ...
  //                   onChange={(x) => setGithubUsername(x.target.value)}
  //                 />
  //               </Socials>
  //             </div>
  ...
  //           {pendingNFTS?.map((x, i) => (
  //             <u key={i}>
  //               {x.competitionName}: {x.pendingAmount}
  //             </u>
  //           ))}
  //           {props.address == address && (
